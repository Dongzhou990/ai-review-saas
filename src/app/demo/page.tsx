...
import { ArrowRight, Zap, Sparkles, Copy, Check, MessageSquare, X } from "lucide-react";
...
const PRESET_EXAMPLES = [
  {
    ...
    rating: 1,
    content:
      "做了个指甲，三天就翘边了。做完颜色也跟色板差很多，沟通了半天也没用。不会再来了，失望。",
  ...
  },
  {
    ...
    rating: 2,
    content:
      "做了一次水光，说实话跟没做一样。1680花得完全不值。而且做完脸上还过敏了，说好的敏感肌专用产品呢？",
    tone: "apologetic" as const,
  },
  {
    id: 3,
    ...
    rating: 3,
    content:
      "剪得还行，但跟我给的参考图效果差很多。沟通的时候说明白了，但理发师好像有自己的想法。价格不算便宜，下次应该换一家了。",
...
export default function DemoPage() {
  ...
  const [tone, setTone] = useState<"professional" | "friendly" | "apologetic" | "enthusiastic">("apologetic");
  ...
  const handleGenerate = async () => {
    ...
    // If limit reached, show payment modal
    ...
    try {
      ...
      if (data.error) {
        ...
        if (!verifiedPhone) {
          ...
          const updated = { count: newCount, date: getToday() };
          setUsage(updated);
          saveUsage(newCount);
        }
      }
  ...
  const handleVerifyPhone = async () => {
    ...
    try {
      ...
      if (data.verified) {
        ...
        setPayPhone("");
        setError("");
        // Auto-generate after verification
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {!verifiedPhone && (
              ...
              >
                ¥199 解锁 <ArrowRight className="w-3 h-3" />
              </Button>
            )}
          </div>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-2xl border border-[#F0E4E0] p-5 space-y-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-[#3D2220]">差评内容</h3>
              </div>

              <textarea
                value={reviewContent}
                onChange={(e) => { setReviewContent(e.target.value); setActivePreset(null); }}
                placeholder="把大众点评/美团上的差评粘贴到这里..."
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  <label className="text-xs text-[#8B6B65] mb-1 block">评分</label>
                  <select
                    value={rating}
                ...
                <div>
                  <label className="text-xs text-[#8B6B65] mb-1 block">语气</label>
                  <select
                    value={tone}
                    onChange={(e) => setTone(e.target.value as any)}
                    className="w-full h-9 rounded-lg border border-[#F0E4E0] px-3 text-sm text-[#3D2220] focus:outline-none focus:ring-2 focus:ring-[#D4725A]/30 bg-[#FFFBF9]"
                  >
                    <option value="apologetic">🙏 真诚道歉</option>
                    <option value="professional">💼 专业得体</option>
                    <option value="friendly">😊 亲切温和</option>
                    <option value="enthusiastic">🎉 热情感激</option>
                  </select>
                </div>
              </div>
              ...
              >
                {loading ? (
                  ...
                  <>
                    <Zap className="w-5 h-5" /> 今日3次已用完，¥199 解锁无限使用
                  </>
                ) : (
                  <>
          ...
          <div>
                ...
                <div className="flex-1 flex items-center justify-center text-[#B8A09A] text-sm">
                  {loading ? (
                    ...
                    <div className="text-center">
                      ...
                      <p>左边选一条差评，或粘贴你自己的</p>
                      <p className="text-xs mt-1">点"生成 AI 回复"看效果</p>
                    </div>
                  )}
                </div>
              )}
            ...
            {/* CTA under output when not verified */}
            {reply && !verifiedPhone && (
              <div className="mt-4 text-center">
                ...
                <Button
                  ...
                  onClick={() => setShowPaymentModal(true)}
                >
                  ¥199 解锁无限使用 <ArrowRight className="w-5 h-5" />
                </Button>
              </div>
            )}
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 relative shadow-2xl animate-in">
            ...
            <div className="text-center mb-6">
              ...
              <h2 className="text-2xl font-bold text-[#3D2220]">
                ¥199 解锁 14 天
              </h2>
              <p className="text-[#8B6B65] text-sm mt-1">无限次 AI 回复 · 不满意退</p>
            </div>
